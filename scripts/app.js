...
const App = () => {
    ...
    const [asesoresCache, setAsesoresCache] = useState({});
    const [lotesCache, setLotesCache] = useState({});
    const [resumenCache, setResumenCache] = useState({});
    ...
    const cargarAsesoresDisponibles = async (hojaSeleccionada) => {
        ...
        if (asesoresCache[hojaSeleccionada]) {
            setAsesoresDisponibles(asesoresCache[hojaSeleccionada]);
            return;
        }
        ...
        try {
            ...
            setAsesoresDisponibles(lista);
            setAsesoresCache((prev) => ({
                ...prev,
                [hojaSeleccionada]: lista
            }));
        } catch (error) {
    ...
    const cargarResumenEjecutivoActual = async (asesorActual, hojaActual, filtroActual) => {
        ...
        const cacheKey = `${asesorActual}__${hojaActual}__${filtroActual}`;

        if (resumenCache[cacheKey]) {
            setResumenEjecutivo(resumenCache[cacheKey]);
            return;
        }
        ...
        try {
            ...
            const resumenNormalizado = {
                ...
                gestionesHoy: data.gestionesHoy || 0
            };
            setResumenEjecutivo(resumenNormalizado);
            setResumenCache((prev) => ({
                ...prev,
                [cacheKey]: resumenNormalizado
            }));
        } catch (error) {
    ...
    useEffect(() => {
        if (!asesor || modo !== 'ASESOR' || !validarUrl()) return;

        const cacheKey = `${asesor}__${hoja}__${filtroEstado}`;

        if (!lotesCache[cacheKey]) {
            obtenerClientesParaGestion(asesor, hoja, filtroEstado)
                .then((data) => {
                    const lista = Array.isArray(data) ? data : [];
                    setLotesCache((prev) => ({
                        ...prev,
                ...
                .catch((error) => {
                    console.error("Error al precargar lote:", error);
                });
        }

        if (!resumenCache[cacheKey]) {
            obtenerResumenEjecutivo(asesor, hoja, filtroEstado)
                .then((data) => {
                    const resumenNormalizado = {
                        totalCargadas: data.totalCargadas || 0,
                        pendientes: data.pendientes || 0,
                ...
                .catch((error) => {
                    console.error("Error al precargar resumen:", error);
                });
        }
    }, [asesor, hoja, filtroEstado, modo, lotesCache, resumenCache]);
    ...
    const iniciarLote = async () => {
        ...
        try {
            const cacheKey = `${asesor}__${hoja}__${filtroEstado}`;
            const data = lotesCache[cacheKey] || await obtenerClientesParaGestion(asesor, hoja, filtroEstado);
    ...
    const guardarGestiones = async ({ gestiones, onSuccess, logLabel, alertMessage }) => {
        ...
        try {
            ...
            setLotesCache({});
            setResumenCache({});
            await cargarResumenEjecutivoActual(asesor, hoja, filtroEstado);
            onSuccess();
            setCargando(false);
            return true;
        } catch (error) {
