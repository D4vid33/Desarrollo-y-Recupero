...
window.CRMApp.components = (() => {
            ...
            <div className={`bg-white rounded-[2rem] p-6 shadow-lg border border-slate-200 ring-1 ring-slate-100 transition-all ${cliente.gestionado ? 'border-indigo-200 ring-indigo-100' : ''}`}>
                <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                        ...
                        <h3 className="font-extrabold text-slate-900 text-sm uppercase leading-tight">{cliente.nombre}</h3>
                        {hoja === 'BD' && cliente.recurrencia !== '' && cliente.recurrencia !== null && cliente.recurrencia !== undefined && (
                            <div className="mt-2 inline-flex items-center px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-[10px] font-black uppercase tracking-widest">
                                Recurrencia: {formatearMoneda(cliente.recurrencia)}
                            </div>
                        )}
                    </div>
    ...
    const ConfigView = ({
        ...
        <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50">
            <div className="w-full max-w-sm bg-white rounded-[2.5rem] shadow-2xl p-8 border border-gray-100 animate-slide-up text-center">
    ...
    const LoteView = ({
        ...
        <div className="max-w-5xl mx-auto p-4 pb-32 animate-slide-up">
            ...
            <div className="relative mb-6">
                <input type="text" placeholder="Buscar en lote actual..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full bg-white border-2 border-transparent focus:border-indigo-500 rounded-[1.5rem] py-4 px-6 shadow-sm outline-none font-semibold text-sm transition-all" />
            </div>
            {loteVisibleBase.length === 0 ? (
                ...
                <div className="space-y-5">
                    {loteVisible.map((cliente, visibleIndex) => {
